...
纯 Rust、无 C 依赖,用 `wasm-pack` 构建(`npm run build:wasm`),产物 `src/wasm/img-ops/` **已提交进仓库**(改 Rust 后手动重建并提交;部署环境无需 Rust 工具链)。依赖 `fast_image_resize`(Lanczos3 + SIMD128)和 `kamadak-exif`。
